<!-- sección home bienvenidos-->
<section class="mt-3 py-5 py-sm-2 py-lg-5" id="bienvenidos">
    <div class="row container mx-auto">
    <div id="slider">
            <div id="carouselExampleCaptions" class="carousel slide">
                <div class="carousel-indicators">
                    <button type="button" data-bs-target="#carouselExampleCaptions" data-bs-slide-to="0" class="active" aria-current="true" aria-label="Slide 1"></button>
                    <button type="button" data-bs-target="#carouselExampleCaptions" data-bs-slide-to="1" aria-label="Slide 2"></button>
                    <button type="button" data-bs-target="#carouselExampleCaptions" data-bs-slide-to="2" aria-label="Slide 3"></button>
                </div>
            <div class="carousel-inner">
                <div class="carousel-item active">
                <img src="./img/slider_1.jpg" class="d-block w-100" alt="foto de estatua de buda">
                <div class="carousel-caption d-none d-md-block background-caption">
                    <h2 class="h2-caption">Decoración</h2>
                    <p>Encontrá los más lindos objetos para decorar tu hogar.</p>
                </div>
                </div>
                <div class="carousel-item">
                <img src="./img/slider_2.jpg" class="d-block w-100" alt="manos de mujer enrollando colchoneta">
                <div class="carousel-caption d-none d-md-block background-caption">
                    <h2 class="h2-caption">Yoga</h2>
                    <p>Conocé las últimas tendencias en equipamiento para la práctica de yoga.</p>
                </div>
                </div>
                <div class="carousel-item">
                <img src="./img/slider_3.jpg" class="d-block w-100" alt="objetos tibetanos para meditación sobre una mesa">
                <div class="carousel-caption d-none d-md-block background-caption">
                    <h2 class="h2-caption">Meditación</h2>
                    <p>Te ofrecemos artículos únicos para que tu experiencia de meditación sea de otro nivel.</p>
                </div>
                </div>
            </div>
            <button class="carousel-control-prev" type="button" data-bs-target="#carouselExampleCaptions" data-bs-slide="prev">
                <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                <span class="visually-hidden">Anterior</span>
            </button>
            <button class="carousel-control-next" type="button" data-bs-target="#carouselExampleCaptions" data-bs-slide="next">
                <span class="carousel-control-next-icon" aria-hidden="true"></span>
                <span class="visually-hidden">Siguiente</span>
            </button>
            </div>
        </div>
        <div class="col-12 col-lg-6 pt-sm-5 pt-lg-0 align-self-center">
            <div class="p-2 p-lg-5 pb-md-0">
                <h2 class="pb-md-3 pb-lg-3">Bienvenidos a la tienda online de Dhara</h2>

                <p><strong>En nuestra tienda encontrarás gran variedad de productos de decoración y todo lo necesario para practicar yoga y meditación.</strong></p>


                <p> <em>¡Explorá entre las diferentes categorías y encontrá el producto perfecto para vos!.</em></p>
                </p>
            </div>
        </div>
        <div class="col-12 col-lg-6 my-sm-3 mb-md-5 py-sm-5 pb-md-2 align-self-center">
            <img src="./img/img-home.jpg" alt="foto de un alohadon zafu junto a un cuenco tibetano"
                class="img-fluid rounded-3 shadow-sm">
        </div>
    </div>
</section>
